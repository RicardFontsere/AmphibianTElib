...
rule SHORT_READ_PREP:
    ...
    Everything runs inside the species RepeatExplorer/ directory.
    """
    input:
        unpack(repeatexplorer_reads),
        genome = os.path.join(GENOMES_DIR_DONE, "{species}", "{species}_headers.fna"),
    output:
        interleaved = os.path.join(GENOMES_DIR_DONE, "{species}", "RepeatExplorer", "reads_interleaved.fasta"),
    params:
        seqtk    = SEQTK,
    ...
    shell:
        ...
        SURVIVED=$(zcat R1.qc.fq.gz | awk 'END{{print NR/4}}')
        ...
        {params.seqtk} sample -s{params.seed} R1.qc.fq.gz "$N_PAIRS" > R1.sub.fq
        {params.seqtk} sample -s{params.seed} R2.qc.fq.gz "$N_PAIRS" > R2.sub.fq
        ...
        {params.seqtk} trimfq -L {params.readlen} R1.sub.fq > R1.trim.fq
        {params.seqtk} trimfq -L {params.readlen} R2.sub.fq > R2.trim.fq

        echo "[$(date)] === Convert to FASTA + rename with coded names ==="
        {params.seqtk} seq -A R1.trim.fq \
          | awk 'BEGIN{{n=0}} /^>/{{n++; printf ">%07d/1\n", n; next}} {{print}}' > R1.fa
        {params.seqtk} seq -A R2.trim.fq \
        ...
        {params.seqtk} mergepe R1.fa R2.fa > {output.interleaved}
...
rule REPEATEXPLORER:
    ...
    shell:
        ...
        apptainer exec --bind "$PWD":/data/ {params.sif} \
          ...
          -v /data/re_output /data/reads_interleaved.fasta
        touch {output.done}
